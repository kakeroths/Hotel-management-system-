...
function SalesChart({bookings, numDays}) {
  ...
  // setting the color of the area chart based on isDarkmode global state
  ...
  : {
    ...
    // All days between today and 7 or 30 or 90 days before depending on the filter
    ...
    const data = allDates.map(date => {
      return {
        ...
        totalSales: bookings.filter(booking => booking.isPaid === true && isSameDay(date, new Date(booking.created_at))).reduce((acc, cur) => acc + cur.totalPrice , 0),
        extrasSales: bookings.filter(booking => booking.isPaid === true && isSameDay(date, new Date(booking.created_at))).reduce((acc, cur) => acc + cur.extrasPrice , 0),
      }
    })
    ...
    <StyledSalesChart>
      ...
      <ResponsiveContainer height={300} width= "100%">
          <AreaChart data={data}>
              ...
              <Tooltip contentStyle={{backgroundColor: colors.background, margin: "1.2rem 4rem"}}/>
              <Area 
              dataKey= "totalSales"
              type= "monotone"
